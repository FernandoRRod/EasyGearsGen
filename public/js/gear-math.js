...
export function sampleArc(radius, startAngle, endAngle, steps) {
  const points = [];
...
export function involuteValue(angle) {
  return Math.tan(angle) - angle;
}

export function pressureAngleAtRadius(baseRadius, radius) {
...
export function recalculateGearModel(source) {
...
export function createGearPath(gear) {
...
export function layoutGears(gears, spacing, width, height) {
...
export function getGearBounds(gears, fallbackWidth, fallbackHeight) {
  if (!gears.length) {
    return {
      minX: 0,
      maxX: fallbackWidth,
